...
const TodoList = () => {
    ...
    return (
        <div>
            <input type="text" ref={todoRef} className='border border-gray-300 px-2 py-2 w-60' required />
            ...
            <div className='mt-4 w-64 mx-auto'>
                {todos.map((item) => (
                    <div key={item.id} className='mb-2'>
                        {item.text}
                        <button onClick={() => removeHandler(item.id)} className='border mx-2 py-1 px-2'>Remove</button>
                    </div>
                ))}
            </div>
        </div>
